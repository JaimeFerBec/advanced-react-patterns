...
This is a very useful pattern to abstract the user of a library/component when they would need to make a series of calls to a dispatcher in a particular order for the library/component to be used correctly. This lowers the probability of the user using the library/component in an unintended manner, or in an erroneous manner, for instance, if the library/component gets updated, our reducer actions' types get updated, so the action's types previously dispatched by the library/component are now wrong.

I would have never thought of passing the dispatcher as an argument to the helper functions, but Dan Abramov's tweet helped understand why that is a better approach. 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=01%3A%20Context%20Module%20Functions&em=jaime.ferbec%40gmail.com).
